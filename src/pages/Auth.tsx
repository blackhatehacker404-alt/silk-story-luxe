    ...
    <main className="min-h-screen bg-background flex items-center justify-center px-4 pt-32 pb-16">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-heading text-foreground">Kalai Fashions</h1>
        ...
        <div className="bg-card border border-border rounded-lg p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            {view === "signup" && (
              <div className="space-y-2">
                <Label className="text-sm font-body tracking-wide uppercase text-muted-foreground">Full Name</Label>
            ...
            {view === "login" && (
              <button type="button" onClick={() => setView("forgot")} className="text-xs text-muted-foreground hover:text-foreground font-body">
          ...
          {view !== "forgot" && (
            <>
              ...
              <Button onClick={() => signInWithGoogle()} variant="outline" className="w-full font-body tracking-wide">
          ...
          <p className="text-center mt-6 text-sm text-muted-foreground font-body">
            {view === "forgot" ? (
              <button onClick={() => setView("login")} className="text-foreground hover:underline font-medium">Back to Sign In</button>
            ) : view === "login" ? (
              <>Don't have an account?{" "}<button onClick={() => setView("signup")} className="text-foreground hover:underline font-medium">Sign Up</button></>
            ) : (
              <>Already have an account?{" "}<button onClick={() => setView("login")} className="text-foreground hover:underline font-medium">Sign In</button></>
            )}
          </p>
        </div>
      </motion.div>
